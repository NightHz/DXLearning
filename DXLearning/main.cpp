...
int dx9_shader_example()
{
	...
	// create shader
	Dx9::VertexShader vs(device, "vs_transform.hlsl");
	if (!vs)
		return 1;
	if (!vs.Enable(device))
		return 1;
	...
	while (true)
	{
		...
		D3DXMATRIX transform = dx9_objects["cube"]->ComputeTransform() * camera_transform;
