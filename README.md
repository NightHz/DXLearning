...
**多渲染目标**：绑定渲染目标数组，在 shader 中可通过 `SV_RenderTargetArrayIndex` 指定渲染的目标索引。

**立方体贴图**：立方体纹理声明为 `TextureCube cube_map` ，该纹理可给出任一方向上的像素，就像“全景”一样。它一般用作环境贴图，可制作天空，也可模拟反射。我们可以通过实时渲染不断更新立方体贴图，但此举开销较大；利用多渲染目标和几何着色器的复制功能，可以实现单次调用就获得整个立方体贴图，但视锥体剔除会大打折扣。另外，对于观察点偏移带来的误差，可先估计环境的距离，来对观察向量进行调整。

**法线贴图**：如其名，用贴图存储法线信息，但要注意此法线定义在切线空间，使用前需做变换。可以通过三角形位置定义切线，也可通过顶点给定切线，后者往往能得到更平滑的结果。
