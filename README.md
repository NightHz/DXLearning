...
使用贴图为不同的像素提供不同的材质、法线等信息。除了贴图，还需要指定采样器，采样器定义了过滤方式与寻址模式。在根签名中可以指定静态采样器，静态采样器直接绑定到寄存器，无需创建设置描述符。
...
有两类混合，对于物件自身的混合/颜色变化，与光照/贴图技术一样，交由高自由度的 shader 完成；对于与渲染目标的混合，则通过 pso 进行有限的设置。
...
在 shader 中，可以通过 `Texture` `Texture2DArray` 访问纹理资源，或通过 `StructuredBuffer` `RWStructuredBuffer` `ConsumeStructuredBuffer` `AppendStructuredBuffer` 等访问缓冲区，具体可参照 [HLSL 数据类型](https://docs.microsoft.com/en-us/windows/win32/direct3dhlsl/dx-graphics-hlsl-data-types)的页面。对于计算着色器，可以使用 `groupshared float4 group_cache[256];` 连接线程组的共享内存。


### 更多技术

在[我的 Rehenz 仓库](https://github.com/NightHz/Rehenz#d3d12)中有部分技术的实现。

**模板**：它是额外为每一个像素点提供信息存储的地方，在像素点被绘制时，能够按照 pso 中所设置进行变化。其拿手好戏为区域识别。通过设置可绘制区域来实现镜面；通过设置单次绘制区域来实现阴影。

**动态索引**：通过诸如 `Texture2D map[4] : register(t1);` 来访问纹理，虽然只显式指定了 `t1` 寄存器，但它实际上是把 4 个元素分别映射到 `t1,t2,t3,t4` ，因此这些寄存器都需要通过根签名绑定。

**实例化**：在核心工作函数 `Drawxxx` 中可指定实例化数量，简单来说就是画同一个网格多次。在 shader 中，可通过 `SV_InstanceID` 分开，因此可以将使用同一网格的物件通过此功能一次性画出，但也需要将他们的所有信息同时上传与绑定，常见会使用结构缓冲区与动态索引。

**视锥体剔除**：通过物件包围球/包围盒与视锥体的相交测试，提前将看不见的物件踢出渲染，降低开销。

**拾取**：将屏幕点通过视锥体转化为射线，再做相交测试。

**多渲染目标**：绑定渲染目标数组，在几何着色器中可通过 `SV_RenderTargetArrayIndex` 指定渲染的目标索引。

**立方体贴图**：立方体纹理声明为 `TextureCube cube_map` ，该纹理可给出任一方向上的像素，就像“全景”一样。它一般用作环境贴图，可制作天空，也可模拟反射折射。我们可以通过实时渲染不断更新立方体贴图，但此举开销较大；利用多渲染目标和几何着色器的复制功能，可以实现单次调用就获得整个立方体贴图，但视锥体剔除会大打折扣。另外，对于观察点偏移带来的误差，可先估计环境的距离，来对观察向量进行调整。

**法线贴图**：如其名，用贴图存储法线信息，但要注意此法线定义在切线空间，使用前需做变换。可以通过三角形位置定义切线，由 u 增加方向为主切线， v 增加方向为副切线；也可通过顶点给定切线，后者往往能得到更平滑的结果。

**位移贴图**：亦称高度图，与曲面细分配合使用，生成凹凸不平的网格。使用不同频率的高度图以不同速度叠加可产生海浪效果。

**阴影贴图**：阴影贴图存储了以光源的视角获得的场景深度信息，在渲染时，我们首先要计算像素在光源视角的位置，利用其位置获得纹理坐标与深度，再与相应的阴影深度进行比较，当像素深度较大时，其不能被光源直接照射。通过深度偏移可以消除阴影粉刺；通过 PCF （百分比渐进过滤）可平滑阴影边缘。

**环境光遮蔽图**：预计算静态物体顶点在场景中的遮蔽率，使用贴图存储，或直接存入顶点信息，可大幅提高环境光的效果。通过向随机的一些方向射出光线，判断距离，来计算遮蔽率，距离越大，遮蔽越小。

**屏幕空间环境光遮蔽(SSAO)**：以相机视角渲染出法线图和深度图，利用此计算出各像素的遮蔽率，然后把它用到正式渲染上；正式渲染时可借助深度图仅绘制深度相同的像素。计算遮蔽率时，首先需要将目标像素点变换到相机(或世界)空间中，然后在一个遮蔽球内随机产生计算点，计算屏幕空间中的位置，再反过来计算实际的遮蔽点，判断遮蔽点是否会挡住目标点。

**屏幕空间反射(SSR)**：同 SSAO 渲染出法线图和深度图，找到每个像素的反射颜色，然后用到正式渲染上。我们要模拟光线在空间中的传播，计算反射最终命中的像素，从历史帧中获取其颜色作为反射颜色。

**骨骼层次结构与蒙皮网格**：定义每块骨骼的相对变换、父骨骼，即获得了一个骨骼层次结构，实际上我们只需定义非根的枢纽（pivot）；蒙皮网格上的顶点与普通网格一样定义相对根原点的位置，一般称此为绑定空间，除此之外，顶点还会受到骨骼移动的影响，因此要为每个顶点定义绑定的骨骼索引及权重；在使用时，我们首先要将顶点从绑定空间变换到骨骼空间（偏移变换），此变换矩阵其实是骨骼初始参数的逆变换，在程序初始化时可以计算存储，然后便可按层次将顶点一路变换到根空间，再对其按不同骨骼插值。
